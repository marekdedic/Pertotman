...
void Pacman::update()
{
	...
	{
		if (((y + 28) > CURRENT->walls[i]->y) and (y < (CURRENT->walls[i]->y + 28)) and ((((x + 32) > CURRENT->walls[i]->x) and (x < CURRENT->walls[i]->x) and ((xPVel - xNVel) > 0)) or ((x < (CURRENT->walls[i]->x + 32)) and (x > CURRENT->walls[i]->x) and ((xPVel - xNVel) < 0))) and xcontrol)
		{
			x -= (xPVel - xNVel) * (DELTA / 1000.f);
			xcontrol = false;
		}
		if (((x + 28) > CURRENT->walls[i]->x) and (x < (CURRENT->walls[i]->x + 28)) and ((((y + 32) > CURRENT->walls[i]->y) and (y < CURRENT->walls[i]->y) and ((yPVel - yNVel) > 0)) or ((y < (CURRENT->walls[i]->y + 32)) and (y > CURRENT->walls[i]->y) and ((yPVel - yNVel) < 0))) and ycontrol)
		{
			y -= (yPVel - yNVel) * (DELTA / 1000.f);
			ycontrol = false;
		}
...
Level::Level(string name)
{
	...
	if (pacmans.size() != 1) {cerr << "Error with loading level: No, or too many pacman found." << endl; QUIT = true; return;}
}
