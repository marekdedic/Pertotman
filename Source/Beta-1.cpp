...
void Bad::update()
{
	...
	for(int i = 0; i < 3; i++)
    {
        if((abs(TransBuff[i]->x - x) + abs(TransBuff[i]->y - y)) < MoveBuff)
        {
            x = TransBuff[i]->x;
            y = TransBuff[i]->y;
            MoveBuff -= ((TransBuff[i]->x - x) + (TransBuff[i]->y - y));
        }
        else
        {
            if((TransBuff[i]->x == RoundTo(x, 32)) and (TransBuff[i]->y < y))
            {
                y -= MoveBuff;
                break;
            }
            ...
            if((TransBuff[i]->y == RoundTo(y, 32)) and (TransBuff[i]->x < x))
            {
                x -= MoveBuff;
                break;
            }
        ...
        if(sqrt((TransBuff[i]->x - x) * (TransBuff[i]->x - x) + (TransBuff[i]->y - y) * (TransBuff[i]->y - y)) <= MoveBuff)
        {
            x = TransBuff[i]->x;
            y = TransBuff[i]->y;
            MoveBuff -= sqrt((TransBuff[i]->x - x) * (TransBuff[i]->x - x) + (TransBuff[i]->y - y) * (TransBuff[i]->y - y));
        }
        else
        {
            cout << "else" << endl;
            x += (MoveBuff * (TransBuff[i]->y - y) / sqrt((TransBuff[i]->x - x) * (TransBuff[i]->x - x) + (TransBuff[i]->y - y) * (TransBuff[i]->y - y)));
            x += (MoveBuff * (TransBuff[i]->x - x) / sqrt((TransBuff[i]->x - x) * (TransBuff[i]->x - x) + (TransBuff[i]->y - y) * (TransBuff[i]->y - y)));
            MoveBuff = 0;
        }
        if (TransBuff[i] == nullptr) {break;}
        if((abs(TransBuff[i]->x - x) + abs(TransBuff[i]->y - y)) < MoveBuff)
        {
            x = TransBuff[i]->x;
            y = TransBuff[i]->y;
            MoveBuff -= abs(TransBuff[i]->x - x) + abs(TransBuff[i]->y - y);
        }
        else
        {
            cout << "else" << endl;
            x += (TransBuff[i]->x - x) * ((TransBuff[i]->x - x) / MoveBuff);
            y += (TransBuff[i]->y - y) * ((TransBuff[i]->y - y) / MoveBuff);
            MoveBuff = 0;
            cout << x << "x" << y << endl;
            cout << "do" << endl;
            break;
    ...
    TransBuff[1] = nullptr;
    TransBuff[2] = nullptr;*/
    x += xVel * ((SDL_GetTicks() - TIME) / 1000.f);
    y += yVel * ((SDL_GetTicks() - TIME) / 1000.f);
    if (((x + 30) > current.characters[0]->x) and (x < (current.characters[0]->x + 30)) and ((y + 30) > current.characters[0]->y) and (y < (current.characters[0]->y + 30)))
	{
		PAUSE = 500;
		REPEAT_LEVEL = true;
...
void Bad::path3()
{
    ...
    if (target == start->down)
    {
        cout << "d" << endl;
        xVel = 0;
        yVel = velocity;
    }
    if (target == start->left)
    {
        cout << "left" << endl;
        yVel = 0;
        xVel = -velocity;
    }
    if (target == start->right)
    {
        cout << "r" << endl;
        yVel = 0;
        xVel = velocity;
